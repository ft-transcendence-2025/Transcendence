...
export class UsernameValidator {
   ...
   */
  static async isAvailable(username: string, checkDatabase: (username: string) => Promise<boolean>): Promise<boolean> {
    const validation = this.validate(username);
    if (!validation.valid) {
      return false;
    }
    return await checkDatabase(this.normalize(username));
  }
}
