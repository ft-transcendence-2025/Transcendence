...
import * as profileService from '../services/User-Management/profile-service';


const profileRoutes: FastifyPluginAsync = async (app: any) => {

	app.post('/:username', { preHandler: [app.authenticate, app.authorize(policy.canViewUser)] }, async (req: FastifyRequest, reply: FastifyReply) => {
		try {
