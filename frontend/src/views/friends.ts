...
export async function renderFriends(container: HTMLElement | null) {
  ...
  try {
    ...
    // Fetch avatars for all friends in parallel
    const friendsWithAvatars = await Promise.all(
      friends.map(async (friend) => ({
        ...friend,
        avatar: await getUserAvatar(friend.username),
      }))
    );

    // Sort friends: ONLINE first, then others
    friendsWithAvatars.sort((a, b) => {
    ...
    for (const friend of friendsWithAvatars) {
      const li = createFriendElement(friend); // No need for async here
...
function createFriendElement(friend: Friend & { avatar: string }): HTMLElement {
  ...
  <span class="relative inline-block w-9 h-9 rounded-full border-2 ${friend.status === "ONLINE"
    ...
    <img src="${friend.avatar}" class="w-9 h-9 object-cover" onerror="this.onerror=null;this.src='assets/avatars/panda.png';"/>
