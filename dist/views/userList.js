...
export async function renderUserList(container) {
    ...
    try {
        ...
        const userListContainer = container.querySelector("#user-list");
        userListContainer.innerHTML = userList
			...
			<div id="user-list" class="bg-gray-100 shadow-md rounded-lg p-4 mb-4 w-2xl">
				...
				<tr>
                        ...
                        <td class="border px-4 py-2 text-left">${user.active ? "Yes" : "No"}</td>
                        <td class="border px-4 py-2 text-left">${new Date(user.createdAt).toLocaleString()}</td>
                    </tr>
			</div>
		`)
            .join("");
